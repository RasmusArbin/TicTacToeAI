...
namespace CSharpGeneralBackendDDotNetCore
{
    ...
    {
        public T RepositoryProvider { get; set; }
        ...
        {
            Bind(repositoryProvider, logger, cache);
        }

        public void Bind(T repositoryProvider, ILogger logger, ICache cache)
        {
        ...
        protected void CacheQuery<T6>(string expression, List<T6> items)
        {
            ...
            if (typeof(T6).GetTypeInfo().GetInterfaces().Contains(typeof(IIdentifiable)))
            {
        ...
        public virtual async Task DeleteAsync<T>(Repository<T> repo, int id)
            where T : class, IIdentifiable
        {
            //Get the item so that we can logg it
        ...
        public virtual async Task UpdateAsync<T>(Repository<T> repo, T entity)
            where T : class, IIdentifiable
        {
